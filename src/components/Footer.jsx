...
const Footer = ({footerAPI:{titles,links,sociallinks}}) => {
  return (
    <>
    <footer className='bg-gradient-to-b from-emerald-400 to bg-green-300 pt-24 pb-7'>
      <div className='grid items-center grid-cols-3 justify-items-center'>
        ...
        {links?.map((list,i)=>(
           ...
           {list.map((val,i)=>(
            <li key={i} className='font-medium text-sm  sm:text-xsm'>{val.link}</li>
           ))}
           </ul>
        ))}
      </div>
      <div className='w-7/12 lg:w-[95vw] m-auto mt-9'>
        ...
        <div className='flex items-center justify-between px-7 md:px-0 md:gap-5 md:flex-col-reverse'>
          <p className='text-sm md:text-center'>CopyRight <sup className='text-base font-bold'>&copy;</sup>All Reserved Rights 2023<span className='font-semibold'>MERN STack Developer</span></p>
